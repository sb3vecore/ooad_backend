...
public class TestDatabaseModel {
    ...
    public boolean checkTeacherId(String teacherId) {
        try {
            ...
            ResultSet resultSet = statement.executeQuery(String.format("SELECT ID FROM Teacher WHERE ID = \"%s\";", teacherId));
    ...
    public int storeQuestions(ArrayList<Question> questions) {
        try {
            Statement statement = this.database.connection.createStatement();
            ResultSet resultSet = statement.executeQuery(String.format("SELECT COUNT(*) AS question_count FROM Questions;"));
    ...
    public void storeTest(int oldQuestionCount, Test test) {
        try {
            ...
            statement.executeUpdate(String.format("INSERT INTO Test VALUES (\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", %s);", 
                "TEST" + count,
                test.getTeacherId(),
                test.getSubject(),
                test.getDifficulty(),
